...
polygons AS (
	SELECT a.id, fid, type, class, b.geom
	FROM polygons_ a
...
edge_points AS (
    SELECT parent as id, cast(path as int) as path, ST_SetSRID(pointg, 28992) as geom FROM ST_DumpPoints((select geom, id from polygonsz)) d
),
...
basepoints AS (
	...
	SELECT id as id, ST_Triangulate2DZ(ST_Collect(geom), 0) as geom FROM line_z
    WHERE
    --ST_IsValid(geom)
